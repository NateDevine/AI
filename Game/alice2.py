...
#
      if type == FEEDBACK_VIEW:
            ...
            #print(self.view,self.didprobe,self.foundplayer)
...
#
         ...
         if (self.x,self.y) != (rem[4][0],rem[4][1]):
            #print('view: ',rem)
            #print(self.x,self.y,rem[5],rem[6])
            #print(self.view,self.didprobe,self.foundplayer)
            self.x,self.y != rem[5],rem[6]
         ...
         for i in range(4):
            d,e = rem[i]
            if isWall(e):
               self.walls[(x+d*dx[i],y+d*dy[i])][i] = True
            elif isPlayer(e):
               self.foundplayer = True
               self.founddir    = i
               self.founddist   = d
               #print('Alice found a player', d, 'units', dirnames[i])
               #print(self.view, self.x, self.y)
         self.didprobe = True
         pass
...
#
   ...
   def action(self):                     # ACTION begins here -=-=-=-=-=-=-=-=-
      ...
      if choice([True,True,False]) and not self.didprobe:
         self.didprobe = True
         return (CMD_PROBE, 0)
