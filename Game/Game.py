...
from alice2 import Robo as Robo1
...
class Game:
   ...
   def __init__(self, win, rows, cols):
      ...
      self.info = Label(self.infoframe,text='Welcome to ISU',width=40,height=1,fg='black',bg='dodgerblue')
   ...
   def getview(self, x, y, pno):
      ...
      vw.append(self.player[pno].x)
      vw.append(self.player[pno].y)
      return vw
      ...
      if cmd == CMD_SHOOT:
         if not self.wall[(x,y)][dir]:
            if isPlayer(self.board[newx][newy]):
               ...
               self.robo[tar].feedback((FEEDBACK_GOTHIT,(rx,ry)))
               self.player[tar].move(rx,ry)
            else:
               print('player', who, 'shoots into an empty room')
               self.robo[who].feedback((FEEDBACK_MISS,0))
         else:
            print('player', who, 'shoots into an empty room')
         ...
         if not self.wall[(x,y)][dir]:
            ...
            print('player',who,'bumps in to a wall')
